...
def run_initiation_wizard(project_type: str = None) -> CharterData:
    ...
    # Basic info
    project_title = questionary.text("Project Title:", validate=lambda text: len(text) > 0).ask()

    project_sponsor = questionary.text("Project Sponsor:", default="Jonathan Ives").ask()

    department = questionary.text(
        "Department/Organization:", default="Texas Pulmonary & Critical Care Consultants"
    ).ask()
    ...
    desired_outcomes = questionary.text(
        "Desired Outcomes:", multiline=True, instruction="(Press ESC then Enter when done)"
    ).ask()
    ...
    cost_benefit_analysis = questionary.text(
        "Cost/Benefit Analysis:", multiline=True, instruction="(Press ESC then Enter when done)"
    ).ask()
    ...
    deliverables = questionary.text(
        "Major Deliverables:", multiline=True, instruction="(Press ESC then Enter when done)"
    ).ask()
    ...
    has_budget = questionary.confirm("Do you have an estimated budget?", default=False).ask()
    ...
    has_duration = questionary.confirm("Do you have an estimated duration?", default=False).ask()
