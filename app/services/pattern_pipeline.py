...
from .ai_agents import DraftAgent, EditorAgent, CriticAgent
...
class PatternPipeline:
    ...
    def execute(
        ...
        pipeline_log = [
            {'stage': 'draft', 'content': draft, 'length': len(draft)}
        ]
        ...
        if enable_critique and pattern['rubric']:
            ...
            for iteration in range(max_revision_iterations):
                ...
                logger.info(f"Pipeline: Critique iteration {iteration + 1} - Score: {score:.2f} (threshold: {threshold})")
                ...
                if iteration < max_revision_iterations - 1:
                    ...
                    final_content = self.editor_agent.edit_draft(
                        final_content,
                        specific_guidance=revision_prompt
                    )
                    pipeline_log.append({
        ...
        metadata = {
            'project_name': self.project_context.get_project_name() if self.project_context else 'Unknown',
        ...
        formatted_doc = self.registry.render_output(
            pattern_name,
            content=final_content,
            **metadata
        )
        ...
        if critique.get('scores'):
            ...
            if low_scores:
                ...
                for score in low_scores[:2]:  # Top 2
                    guidance += f"- **{score['criterion']}**: {', '.join(score.get('improvements', []))}\n"
