...
Parses work plan documents and exports to OpenProject via API.
Creates project structure with phases (parent work packages), tasks (child work packages), and milestones.

Usage:
    python scripts/export_to_openproject.py path/to/ISSUES.md --api-key YOUR_API_KEY --url http://10.69.1.86:8080
...
Example:
    python scripts/export_to_openproject.py docs/ISSUES.md
"""
...
class OpenProjectExporter:
    ...
    def parse_issues_md(self, filepath: str) -> Dict:
        """Parse work plan document and extract project structure"""
    ...
    def _extract_project_metadata(self, content: str) -> Dict:
        """Extract start date, due date, priority from project structure"""
        metadata = {}
...
def main():
    parser = argparse.ArgumentParser(description='Export work plan to OpenProject')
    parser.add_argument('issues_file', help='Path to work plan file (ISSUES.md)')
